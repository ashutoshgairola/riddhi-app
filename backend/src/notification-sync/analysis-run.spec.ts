...
import { AccountType, NotificationType, PaymentMethod, TransactionType } from '../common/enums';
...
describe('runAnalysisForUser', () => {
  it('turns a correlated group into one pending detection, marks captures analyzed, pushes', async () => {
    ...
    const analysis = { analyze: jest.fn(async () => [{
      ...
      institution: 'HDFC', rail: 'upi', last4: null, confidence: 0.9, sourceKeys: ['k-rapido', 'k-hdfc'],
    }]) };
  ...
  it('no captures → no analyze call, no push', async () => {
    ...
    const moduleRef = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(CreditCard), useValue: { find: jest.fn() } },
        { provide: NotificationAnalysisService, useValue: analysis },
        { provide: NotificationsService, useValue: notifications },
  ...
  it('suppresses a detection whose charge already exists on the resolved account (reverse dedup)', async () => {
    ...
    const captures: any[] = [
      { id: 'c1', dedupKey: 'k-hdfc', packageName: 'com.hdfc', title: '', text: 'Rs.499 spent on card *1234', postedAt: posted, analyzed: false },
    ];
    const savedDetections: any[] = [];
    const capRepo = { find: jest.fn(async () => captures), update: jest.fn(async () => undefined) };
    const detRepo = { create: (x: any) => x, save: jest.fn(async (x: any) => { savedDetections.push(x); return x; }) };
    // One HDFC credit account, whose card carries last4 1234 → resolves by last4.
    const accRepo = { find: jest.fn(async () => [{ id: 'cc1', institutionName: 'HDFC Bank', type: AccountType.CREDIT }] as Account[]) };
    const cardRepo = { find: jest.fn(async () => [{ accountId: 'cc1', last4: '1234' }] as CreditCard[]) };
    const analysis = { analyze: jest.fn(async () => [{
      merchant: 'Swiggy', amount: 499, type: 'expense', category: 'Food',
      institution: 'HDFC', rail: 'card', last4: '1234', confidence: 0.9, sourceKeys: ['k-hdfc'],
    }]) };
    const notifications = { create: jest.fn(async () => ({})) };
    // An existing debit of 499 on the same account within the window → duplicate.
    const transactions = {
      findForAccountInRange: jest.fn(async () => [
        { id: 't1', date: new Date('2026-06-12T09:00:00Z'), amount: 499, type: TransactionType.EXPENSE, accountId: 'cc1', description: 'SWIGGY', importFingerprint: null },
      ]),
    };
  ...
  it('keeps a detection when no existing account txn matches (reverse dedup miss)', async () => {
    ...
    const transactions = {
      findForAccountInRange: jest.fn(async () => [
        { id: 't1', date: new Date('2026-06-12T09:00:00Z'), amount: 42, type: TransactionType.EXPENSE, accountId: 'cc1', description: 'OTHER', importFingerprint: null },
      ]),
    };
    ...
    expect(savedDetections[0]).toMatchObject({ merchant: 'Swiggy', amount: 499, accountId: 'cc1' });
  });
});
