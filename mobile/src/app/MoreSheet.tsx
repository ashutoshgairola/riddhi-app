...
const ITEMS: MoreItem[] = [
  ...
  { id: 'accounts', l: 'Accounts', i: '💳', c: '#8197c4', d: 'Banks & wallets' },
  { id: 'tx-cats', l: 'Categories', i: '🏷', c: '#c9a86a', d: 'Manage spending categories' },
  { id: 'notifs', l: 'Notifications', i: '🔔', c: '#c97d8c', d: 'All alerts & updates' },
  { id: 'settings', l: 'Settings', i: '⚙', c: '#8a8299', d: 'Preferences & account' },
];
...
export function MoreSheet() {
  ...
  return (
    <BottomSheet open={moreOpen} onClose={onClose} title="More">
      <View style={styles.grid}>
        {cards.map((it) => (
          <Pressable
            ...
            style={[styles.card, { backgroundColor: t.glassBg, borderColor: t.glassBrd }]}
          >
            <View style={[styles.cardIcon, { backgroundColor: `${it.c}22` }]}>
              <Text style={[styles.cardIconGlyph, { color: it.c }]}>{it.i}</Text>
            </View>
            <View>
              <Text style={[styles.cardLabel, { color: t.text1, fontFamily: weight(700) }]}>{it.l}</Text>
              <Text style={[styles.cardDesc, { color: t.text3, fontFamily: weight(500) }]}>{it.d}</Text>
            </View>
          </Pressable>
        ))}
      </View>
      ...
      <ListCard>
        {rows.map((it, i) => (
          <ListRow key={it.id} onPress={() => handlePress(it.id)} last={i === rows.length - 1}>
            <View style={[styles.rowIcon, { backgroundColor: `${it.c}22` }]}>
              <Text style={[styles.rowIconGlyph, { color: it.c }]}>{it.i}</Text>
            </View>
            <View style={styles.rowText}>
              <Text style={[styles.rowLabel, { color: t.text1, fontFamily: weight(600) }]}>{it.l}</Text>
              <Text style={[styles.rowDesc, { color: t.text3, fontFamily: weight(500) }]}>{it.d}</Text>
            </View>
